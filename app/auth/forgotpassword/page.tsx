...
export default function ForgotPasswordPage() {
    ...
    const handleEmailSubmit = async (e) => {
        ...
        try {
            await axios.post("/api/auth/resetresend", { email });
            showPopup('Reset code sent to your email!', 'success');
            setCurrentStep('verify');
            setResendTimer(60);
        } catch (error) {
            console.log(error.message);
            showPopup('Failed to send reset code. Please try again.', 'error');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const verifyCode = async (codeToVerify = code.join('')) => {
        ...
        try {
            ...
            setVerificationCode(codeToVerify); // Store the verified code
            setCurrentStep('reset'); // Move to password reset step
        } catch (error) {
            console.log(error.message);
                        ...
                        <form onSubmit={handleEmailSubmit}>
                            <div className="mb-8">
                                <input
                                    ...
                                    className="w-full px-4 py-4 text-lg border-2 border-gray-300 rounded-xl text-gray-800 outline-none transition-all duration-300 focus:border-indigo-500 focus:scale-105 focus:shadow-md"
                                    required
                                    autoFocus
                                />
                            </div>
